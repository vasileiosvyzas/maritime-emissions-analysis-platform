...
def fetch_ship_ids() -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        unique_ships = wr.athena.read_sql_query(query, database=DATABASE, boto3_session=aws_session)
        logger.info(f"Fetched all the ships from the database. There are {unique_ships.shape[0]} ships in the database.")
