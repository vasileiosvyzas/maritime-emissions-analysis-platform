...
spark_df = spark_df.drop(*('d.1', 'verifier address', 'additional information to facilitate the understanding of the reported average operational energy efficiency indicators')) 
...
def convert_columns_to_double(df, columns):
    for column in columns:
        ...
        if dict(df.dtypes)[column] == 'string':
            ...
            df = df.fillna(0.0, subset=[column])
